...
public class NfcParser {
    ...
    public NfcParser(Activity activity,MainMenuReadWriteModel model){
    ...
    public void parseNfc(String result) throws SensorTimeException {
        ...
        model.addCurrentReading(now, currentReading);
        ...
        BGReading last = model.getMostRecentHistoryReading();
        for(Calendar c: historyMap.keySet()){
            ...
            if(newSensor||last==null||(last!=null&&historyMap.get(c)>0.01&&c.after(getSLastscan()))){
                model.addHistoryReading(c, historyMap.get(c));
