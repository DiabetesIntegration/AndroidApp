package com.example.kbb12.dms.database.bolusInsulinModel;
...
public class BolusInsulinModel implements IBolusInsulinModel {
    ...
    public void improveICRValue(Calendar time, double percentageChange) {
        ...
        cascadeICRImprove(temp, percentageChange*STEP_DOWN);
        temp.add(Calendar.HOUR,-1);
        cascadeICRImprove(temp,percentageChange*TWO_STEP_DOWN);
    ...
    private void cascadeICRImprove(Calendar time,double percentageChange){
        if(percentageChange>0.1){
            //Limit percentageChange to 10% for safety. This shouldn't be needed here because of the
            //level above but it's still better to be over cautious with health issues.
            percentageChange=0.1;
        }
        Float currentValue = getICRValue(time,true);
        Float improvement = currentValue*(float)percentageChange;
