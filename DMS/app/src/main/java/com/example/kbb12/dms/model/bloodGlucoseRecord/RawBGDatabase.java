...
import com.example.kbb12.dms.nfc.DatabaseHelper;
...
public class RawBGDatabase {
    ...
    public long addRawData(String rawData, Calendar timestamp) {
        ...
        values.put(RawDataContract.COLUMN_NAME_DATA, rawData);
        values.put(RawDataContract.COLUMN_NAME_TIME, getDateTime(timestamp));

        // insert row
        long id = write.insert(RawDataContract.TABLE_NAME, null, values);
    ...
    public Map<Calendar, String> getAllBasicData(){
        ...
        String selectQuery = "SELECT  * FROM " + RawDataContract.TABLE_NAME;
        ...
        if (c.moveToFirst()) {
            do {
                basicData.put(parseCalendar(c.getString(c.getColumnIndex(RawDataContract.COLUMN_NAME_TIME))), c.getString(c.getColumnIndex(RawDataContract.COLUMN_NAME_DATA)));
            } while (c.moveToNext());
        }
