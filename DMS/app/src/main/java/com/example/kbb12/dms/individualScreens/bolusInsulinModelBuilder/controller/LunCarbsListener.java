 ...
 */
public class LunCarbsListener implements TextWatcher {

    private BolusInsulinReadWriteModel model;

    public LunCarbsListener(BolusInsulinReadWriteModel model){
        this.model=model;
    }
    @Override
    ...
    public void afterTextChanged(Editable s) {
        if(s.toString().length()>0) {
            model.setLunCarbs(Double.parseDouble(s.toString()));
        }else {
